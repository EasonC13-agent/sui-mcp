...
server.tool(
  ...
  async ({ network }) => {
    const result = await runCommand(
      `curl -s -X POST ${WALLET_SERVER}/network -H "Content-Type: application/json" -d '{"network": "${network}"}'`
    );
    return { content: [{ type: "text", text: result }] };
  }
);
...
server.tool(
  ...
  async ({ txBytes }) => {
    const result = await runCommand(
      `curl -s -X POST ${WALLET_SERVER}/sign-and-execute -H "Content-Type: application/json" -d '{"txBytes": "${txBytes}"}'`
    );
    return { content: [{ type: "text", text: result }] };
  }
);
...
server.tool(
  "sui_cli",
  "Run any Sui CLI command (e.g., 'client gas', 'move build', 'client object <id>')",
  { 
...
server.tool(
  ...
  { packagePath: z.string().describe("Path to the Move package directory") },
  async ({ packagePath }) => {
...
server.tool(
  ...
  { packageId: z.string().describe("The package ID to view") },
  async ({ packageId }) => {
